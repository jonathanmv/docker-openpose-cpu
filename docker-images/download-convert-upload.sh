...
# Converts a file in s3 to .avi and uploads it to the specified bucket using the same key
# Expects 2 positional params:
# $1 file to be downloaded from s3
# $2 file name in s3 destination bucket

# example: 
# $ ./download-convert-upload.sh s3://original-bucket/convert.mp4 s3://original-bucket/convert.avi
